...
\begin{itemize}
    ...
    \item \url{https://www.img2go.com/colorize-image} - This site can be used as a comparison benchmark for image coloring.
    \item \url{https://picfix.ai/} - More advanced solution for image processing with a bunch of features.
    \item \url{https://vanceai.com/image-enhancer/} - Another site for increasing picture resolution and quality.
    \item \url{https://www.myheritage.com/photo-enhancer} - One more solution, which is focused on faces and family photos.
\end{itemize}
...
\par We want to compare the results of models right out of the box and after fine-tuning from our side. The good result would be to achieve stable and solid relative to the original (out of the box) improvement in accuracy, Structural similarity index  measure (SSIM), Peak signal-to-noise ratio (PSNR), and to have at least \(6/10\) positive feedbacks from users on average.
